...
class DMComponent:
    ...
    """
    def __init__(self, name, reference, frame, coordinates, mean, sigma, errors={'mean':np.zeros(3), 'sigma':np.zeros(3)}, truncated=True, path=None, N_esc=1.):
        """
        ...
        coordinates : (n-dimensions (int), 'system' (str)), e.g. (3, 'cartesian'), (3, 'cylindrical'), (2, 'spherical')
        ...
        path : the path to the files with the precomputed data (default: None)
        N_esc : normalization of the velocity distribution, obtained by integrating it over phase space (default: 1).
        """
    ...
    @property # property decorator (wrapper)
    def N_esc(self):
        """
        Returns the normalization factor of the distribution.
        """

        # calling the 'getter' of N_esc
        return self._N_esc

    @N_esc.setter # setter decorator (wrapper)
    def N_esc(self, value):
        """
        Sets the normalization factor of the distribution to 'value'.
        """

        # calling the 'setter' of N_esc
        self._N_esc = value
    ...
    def fv(self, v, vboost=v_sun, taken_from='3D', arrays=(alpha_arr, beta_arr)):
        ...
        if taken_from == '3D':
        ...
        elif (taken_from == '2D') and (self._coordinates[0] == 2):
            ...
            raise AttributeError("The fv method with \'taken_from\'=\'2D\' cannot be used for this instance of DMComponent, since its value for the self._coordinates[0] attribute is not 2: it is not an isotropic distribution!")
    ...
    def moment_fv(self, vmin, moment, vboost=v_sun, taken_from='3D', arrays=(v_arr, alpha_arr, beta_arr)):
        ...
        if taken_from == '3D':
            ...
            raise AttributeError("The moment_fv method with \'taken_from\'=\'2D\' cannot be used for this instance of DMComponent, since its value for the self._coordinates[0] attribute is not 2: it is not an isotropic distribution!")
...
# test_Gaia_halo = DMVelDistr('Gaia_halo','1807.02519 Necib et al. & files of sampled distributions','galacto-centric',(3, 'spherical'),np.zeros(3),np.array([143.96, 132.03, 118.30]),errors={'mean':(np.zeros(3), -np.zeros(3)), 'sigma':(np.array([4.31, 4.30, 3.42]), np.array([-5.03, -3.57, -1.86]))},truncated=True,path=fileDirectory+"DM_Velocity_Distribution-master/f_v_halo_normalized.txt")
